...
{
    /// <summary>
    /// The converter class for \thanks.
    /// </summary>
    internal sealed class ThanksConverter : CommandConverter
    {
        /// <summary>
        ...
        public override string Convert(LatexExpression expr)
        {
            if (expr.Expressions != null)
            {
                return "<div class=\"thanks\">" +
                       SequenceConverter.ConvertOutline(expr.Expressions[0], expr.Customization) + "</div>\n";
            }
            return "";
        }
    }
